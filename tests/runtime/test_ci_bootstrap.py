...
def test_ci_bootstrap_restores_verified_protocol_and_backend_dependencies() -> None:
    ...
    assert "$protocolVersion = '2.3.0'" in script
    assert "$protocolVersion = '2.3.0'" in release_script
    assert '"vibeocr-runtime-contracts==2.3.0"' in package_config
    assert lock["version"] == "2.3.0"
