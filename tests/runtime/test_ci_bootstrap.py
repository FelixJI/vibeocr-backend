...
def test_ci_bootstrap_restores_verified_protocol_and_backend_dependencies() -> None:
    ...
    assert '"vibeocr-runtime-contracts>=2.8.1,<3.0.0"' in package_config
    assert lock["version"] == "2.8.1"
    assert "vibeocr_runtime_client-2.8.1-py3-none-any.whl" in lock["artifacts"]
    assert "vibeocr_runtime_contracts-2.8.1-py3-none-any.whl" in lock["artifacts"]
